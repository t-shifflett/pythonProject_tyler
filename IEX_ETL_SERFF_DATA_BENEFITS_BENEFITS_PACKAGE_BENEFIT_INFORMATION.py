# title: IEX_ETL_SERFF_DATA_BENEFITS_BENEFITS_PACKAGE_BENEFIT_INFORMATION.py
...
sheet_name = 'BENEFITS_PACKAGE_BENEFIT_INFORMATION'
        ...
        if name.endswith(variables.file_extensions):
            if name.__contains__(file_name):
                ...
                if root.__contains__('2O22'):
                    col5_value = 'NaN'
                # else:
                #     col5_value = col5_value

                excel_read_df = pd.concat(pd.read_excel(root_name, sheet_name=filtered_sheet_list, header=None))

                header_loc = pd.DataFrame(
                    [excel_read_df[excel_read_df[excel_read_df.columns[0]] == 'Benefits'].index[0]])

                excel_read_df = pd.concat(
                    pd.read_excel(root_name, sheet_name=filtered_sheet_list, header=header_loc.iloc[0, 1]))
...
all_data.columns = [
    ...
    'benefits',
    'ehb',
    'is_this_benefit_covered',
    'quantitative_limit_on_service',
    'limit_quantity',
    'limit_unit',
    'exclusions',
    'benefit_explanation',
    'ehb_variance_reason',
    'excluded_from_in_network_moop',
