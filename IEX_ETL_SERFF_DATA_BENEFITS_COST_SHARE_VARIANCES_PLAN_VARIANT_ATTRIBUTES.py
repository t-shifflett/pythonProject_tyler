...
sheet_table_name = 'PLAN_VARIANT_ATTRIBUTES'
            ...
            if name.__contains__(file_name):
                if root.__contains__('2022'):
                    ...
                    header_loc = pd.DataFrame(
                        [excel_read_df[excel_read_df[excel_read_df.columns[0]] == 'Plan Cost Sharing Attributes'].index[0]])

                    excel_read_df = pd.concat(
                        pd.read_excel(root_name,  sheet_name=filtered_sheet_list,  header=(header_loc.iloc[0,  1]+1),
                                      usecols='A:DG'))
