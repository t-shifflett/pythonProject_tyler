...
source = 'SERFF'
file_name = r'DATA_SERVICE_AREA'
excel_sheet_names = ['Service Areas', 'Service', 'Areas']
sheet_name = 'SERVICE_AREAS'
        ...
        if name.endswith(variables.file_extensions):
            if name.__contains__(file_name):
                ...
                excel_read_df = pd.concat(pd.read_excel(root_name, sheet_name=filtered_sheet_list,
                                                        header=10, skiprows=[11]), ignore_index=True)
...
all_data.columns = [
    ...
    'change_by',
    'load_dt',
    'partial_county_justification_filename'
]
