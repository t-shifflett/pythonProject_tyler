...
# this file location: https://github.optum.com/IEX/IEX_PYTHON/tree/master
...
td_table_pk = 'plan_yr, serff_tracking_number, issuer_state, hios_issuer_id, rating_method, plan_id, ' \
              'rating_area_id, tobacco, age '
            ...
            if root.__contains__('2022'):
                if name.__contains__(file_name):
                    ...
                    header_loc = pd.DataFrame(
                        [excel_read_df[excel_read_df[excel_read_df.columns[0]] == 'Plan ID*'].index[0]])

                    # print(header_loc.iloc[0, 1])

                    excel_read_df = pd.concat(
                        pd.read_excel(root_name,
                                      sheet_name=filtered_sheet_list,
                                      header=header_loc.iloc[0, 1],
                                      skiprows=[(header_loc.iloc[0, 1]+1)]))
